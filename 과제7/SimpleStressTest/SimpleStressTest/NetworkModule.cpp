...
void Adjust_Number_Of_Client()
{
	...
	if (DELAY_LIMIT2 < t_delay) {
		...
		client_to_close++;
		return;
	} else // 동접수가 리미트를 넘어섰을때는 접속을 안받고 클라수를 줄여야 함
		// 클라 수를 줄여야 하는 이유는 동접을 늘리지 않아도 딜레이는 늘어날 수 있다
		// 그래서 최대치에서 서서히 줄여가며 최대 동접을 찾아야 함
...
// 그래픽 모듈에서 캐릭터가 어디 분포하는지 알려주는 함수 
// 현재 동접이 얼마이고 각각의 캐릭터가 어디위치하는 지 알려줌
{
	int index = 0;
	for (int i = 0; i < num_connections; ++i)
